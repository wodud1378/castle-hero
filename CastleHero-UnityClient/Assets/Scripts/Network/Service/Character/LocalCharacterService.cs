...
namespace RGLabs.Network.Service.Character
{
    public class LocalCharacterService : ICharacterService
    {
        public UniTask<LevelUpResult> LevelUp(UnitInfo unit, ConsumableItem item, int consumeQuantity)
        {
            ...
            {
                var characters = LocalKeys.Characters.LoadAsArray<UnitInfo>().ToList();
                var items = LocalKeys.Inventory.LoadAsArray<IItem>().ToList();
        ...
        public UniTask<FieldCharacter[]> LoadFormation()=> UniTask.FromResult(LocalKeys.Formation.LoadAsArray<FieldCharacter>());

        public UniTask SaveFormation(IEnumerable<FieldCharacter> characters)
        {
            LocalKeys.Formation.SaveAsArray(characters);
