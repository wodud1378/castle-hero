using System;
using System.Collections.Generic;
using RGLabs.Data.DB;
using RGLabs.Data.Model;
using UniRx;
using Unity.Collections;
using Unity.Jobs;
using Unity.Mathematics;
using UnityEngine;
using Random = UnityEngine.Random;

namespace RGLabs.InGame.System.Wave.Creation
{
    public class OptimizedCreationHelper : ICreationHelper
    {
        [BurstCompatible]
        private struct SetSpaceBuffer : IJob
        {
            public NativeArray<Unit> units;
            public NativeArray<UnitSpace> spaces;

            public float2 cornerA;
            public float2 cornerB;

            private int _entityCount;
            private int _blankCount;
            private int _total;
            private float2 _left;
            private Vector2 _leftSpace;

            public void Execute()
            {
                _entityCount = units.Length;
                _blankCount = Random.Range(2, 4);
                _total = _entityCount + _blankCount;
                _leftSpace = cornerB - cornerA;

                ApplyEntities();
                if (_left is not { x: > 0, y: > 0 })
                    return;

                ApplyBlanks();
            }

            private void ApplyEntities()
            {
                var direction = _leftSpace.normalized;
                spaces = new NativeArray<UnitSpace>(_total, Allocator.Temp);
                for (int i = 0; i < _entityCount; ++i)
                {
                    var unit = units[i];
                    var size = new Vector2(unit.size, unit.size) * direction;
                    _leftSpace -= size;

                    spaces[i] = new UnitSpace
                    {
                        valid = true,
                        index = i,
                        size = size
                    };
                }
            }

            private void ApplyBlanks()
            {
                var blank = _leftSpace / _blankCount;
                for (int i = _entityCount; i < _total; ++i)
                {
                    spaces[i] = new UnitSpace
                    {
                        valid = false,
                        index = -1,
                        size = blank
                    };
                }
            }
        }

        [BurstCompatible]
        private struct SetCreationBuffer : IJob
        {
            public NativeArray<Unit> units;
            public NativeArray<UnitSpace> spaces;
            public Vector2 startPosition;

            public NativeList<UnitCreation> creations;

            public void Execute()
            {
                var indices = new NativeList<int>(Allocator.Temp);
                int count = spaces.Length;
                for (int i = 0; i < count; ++i)
                {
                    indices.Add(i);
                }

                creations = new NativeList<UnitCreation>(Allocator.Temp);

                Vector2 lastPosition = startPosition;
                Vector2 lastHalfSize = default;
                while (indices.Length > 0)
                {
                    int spaceIndex = Random.Range(0, indices.Length);
                    indices.RemoveAt(spaceIndex);
                    
                    var space = spaces[spaceIndex];
                    var halfSize = space.size * 0.5f;
                    var position = lastPosition + lastHalfSize + halfSize;;
                    if (!space.valid)
                        continue;
                    
                    creations.Add(new UnitCreation
                    {
                        id = units[space.index].id,
                        lv = units[space.index].lv,
                        position = position
                    });

                    lastPosition = position;
                    lastHalfSize = halfSize;
                }
            }
        }

        private struct Unit
        {
            public int lv;
            public int id;
            public float size;
        }

        private struct UnitSpace
        {
            public bool valid;
            public int index;
            public Vector2 size;
        }
        
        private readonly int _areaId;
        private readonly float _size;
        private readonly Vector2 _offset;

        private readonly UnitDB _db;
        private readonly Queue<SpawnEvent> _queue;

        private SetSpaceBuffer _setSpaceJob;
        private SetCreationBuffer _setCreationJob;

        public OptimizedCreationHelper(int areaId, UnitDB db, Vector2 cornerA, Vector2 cornerB)
        {
            _setSpaceJob = new()
            {
                cornerA = cornerA,
                cornerB = cornerB,
            };

            _setCreationJob = new()
            {
                startPosition = cornerA
            };

            _areaId = areaId;
            _db = db;

            _queue = new();

            MessageBroker.Default.Receive<SpawnEvent[]>().Subscribe(OnReceiveSpawnEvents);
        }

        public void SetUpCreations(Action<UnitCreation> onCreation)
        {
            if (_queue.Count == 0)
                return;

            var units = GetUnitArray();
            _setSpaceJob.units = units;

            var setSpaceHandle = _setSpaceJob.Schedule();
            _setCreationJob.units = units;
            _setCreationJob.spaces = _setSpaceJob.spaces;

            var setCreationHandle = _setCreationJob.Schedule(setSpaceHandle);
            setCreationHandle.Complete();

            var creations = _setCreationJob.creations;
            int count = creations.Length;
            for (int i = 0; i < count; ++i)
            {
                onCreation.Invoke(creations[i]);
            }
        }

        private void OnReceiveSpawnEvents(SpawnEvent[] data)
        {
            foreach (var ev in data)
            {
                if (ev.area == _areaId)
                    _queue.Enqueue(ev);
            }
        }

        private NativeArray<Unit> GetUnitArray()
        {
            int count = _queue.Count;
            int left = count;
            int i = 0;
            var array = new NativeArray<Unit>(count, Allocator.Temp);
            while (left > 0)
            {
                var data = _queue.Dequeue();
                array[i++] = new Unit
                {
                    lv = data.lv,
                    id = data.id,
                    size = _db.sizeCache.GetValueOrDefault(data.id, 0.3f) * 2f
                };
                
                --left;
            }
            
            return array;
        }
    }
}